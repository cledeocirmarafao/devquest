const loadUsers = async (username) => {
    ...
    if (!response.ok) {
        throw new Error(`Github API returnedd status code ${response.status}`)
    }
...
const loadAll = async () => {
    try {
        const results = await Promise.all([
            loadUsers('cledeocirmarafao'),
            loadRepositories('cledeocirmarafao'),
            loadEvents('cledeocirmarafao')
        ])
        console.log(results)
